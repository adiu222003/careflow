...
class DoctorService:
    ...
    async def get_availability(
        ...
        stmt = select(DoctorProfile).options(
            selectinload(DoctorProfile.working_hours),
            selectinload(DoctorProfile.leaves).where(  # type: ignore
        ...
        range_start = datetime.combine(start_date, time.min).replace(tzinfo=UTC)
        range_end = datetime.combine(end_date, time.max).replace(tzinfo=UTC)
        ...
        holds_stmt = select(AppointmentHold).where(
            ...
            AppointmentHold.expires_at > datetime.now(UTC),
        ...
        while current_date <= end_date:
            ...
            slot_start = datetime.combine(current_date, wh.start_time).replace(tzinfo=UTC)
            slot_end = slot_start + slot_duration
            day_end = datetime.combine(current_date, wh.end_time).replace(tzinfo=UTC)
            ...
            while slot_end <= day_end:
                ...
                day_data["slots"].append({  # type: ignore
